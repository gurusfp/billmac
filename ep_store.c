...
ep_store_init(void)
{
  ...
  EEPROM_STORE_READ((uint16_t)&(EEPROM_DATA.eeprom_sig), (uint8_t *)&ui1, sizeof(uint16_t));
  if (eeprom_crc == ui1) /* we are good */
    return;
  ...
  eeprom_crc = CRC16_High; eeprom_crc <<= 8; eeprom_crc |= CRC16_Low;
  EEPROM_STORE_WRITE((uint16_t)&(EEPROM_DATA.eeprom_sig), (uint8_t *)&eeprom_crc, sizeof(uint16_t));
