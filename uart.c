...
void
uart_putc(char c)  // Put Character to UART.
{
        while(TI==0);
        TI=0;
        SBUF=c;
...
void
uart_puts(char *s) // Put String to UART, (calls uart_putc()).
{
        while(*s!=0)
        {
                uart_putc(*s);
...
void
uart_gets(char *s) // Get String from UART, (calls uart_getce()).
{
...
void
i2s(int i,char *s) // Convert Interger to String.
{
        char sign;
        short len;
        char *p;
...
int
s2i(char *s)                // Convert String to Integer.
{
        int i=0;
        char sign='+';
